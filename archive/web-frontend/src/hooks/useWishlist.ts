...
/** The wishlist is fetched once and shared: cards derive their heart state from it. */
export function useWishlist() {
  ...
  return { query: q, items, has: (id: number) => ids.has(id) };
}

/**
 * Optimistic add/remove: the heart flips instantly, the request runs in the background, and on
 * failure we roll back and tell the user. `onSettled` re-syncs with the server either way.
 */
