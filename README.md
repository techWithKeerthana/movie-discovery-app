...
| Production / no local backend | `https://trackzio-backend.onrender.com` (live; this is what the committed `.env` uses) |

Notes and troubleshooting:
- **Every screen says "Movie service is temporarily unavailable":** the app reached the backend, but the backend cannot reach TMDB (VPN off, ISP block, or bad token). On start the backend prints `TMDB token present: true/false`; `curl https://api.themoviedb.org/3/configuration` from the backend machine should answer `401` (reachable) rather than hang. `npm run verify:tmdb` gives the full picture.
...
- **Phone cannot reach the API:** open `http://<LAN IP>:4000/api/health` in the phone's browser. If it fails, allow Node.js through the Windows firewall for private networks. The backend listens on all interfaces; CORS is irrelevant to a native app, but does gate the web build — see `CORS_ORIGIN` in `backend/.env.example`.
...
| **Render free tier sleeps after ~15 min idle** | The first request after a while wakes the instance, taking roughly 30-60s; the app's existing loading and slow-network states (`SlowHint`, skeletons) cover this without special-casing it, but it is a real, noticeable delay a reviewer will hit | **Accepted**: normal free-tier behaviour; a paid Render plan removes it | Upgrading the Render plan removes the sleep; no code change |
| **Turso's remote protocol silently differs from local SQLite for some statements** | Found deploying this: `PRAGMA user_version` (used for schema versioning) can be read remotely but not written — a real gap between the `file:` and `libsql://` transports that only surfaced against the live database, not in tests against a local file. Fixed by tracking migrations in a plain table instead (works identically both ways) | **Fixed**, and verified directly against the live Turso database and via a real Render deploy + restart (wishlist entry survived) | — |
