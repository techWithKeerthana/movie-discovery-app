 ...
 */
export function MovieList({ data, header, empty, footer, onEndReached, refreshing = false, onRefresh, dim = false, listRef, testID }: Props) {
  ...
  const renderItem: ListRenderItem<MovieSummary> = useCallback(
    ({ item }) => <MovieCard movie={item} width={cardWidth} wished={ids.has(item.id)} onOpen={onOpen} onToggle={onToggle} />,
    [cardWidth, ids, onOpen, onToggle],
  );
